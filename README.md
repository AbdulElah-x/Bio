```markdown
# 👋 Hi, I’m Abdulelah AlQahtani


-🔍 **Reverse Engineering** enthusiast: IDA Pro, Ghidra, x64dbg
-📱 **App Developer**: Python, C++, C#, Flutter
-💻 **Full‑Stack Programmer**: JavaScript, Node.js, React
-🛡️ **Certified Cybersecurity Professional**: CEH, CISSP, CompTIA Security+
-🏆 Passionate about solving complex problems and sharing knowledge

---


| Category               | Skills & Tools                            |
|------------------------|-------------------------------------------|
| Reverse Engineering    | IDA Pro, Ghidra, Radare2, x64dbg          |
| Programming Languages  | Python, C, C++, C#, JavaScript, Dart      |
| Frameworks & Platforms | .NET, Flutter, Node.js, React, Flask      |
| Security & Pentesting  | Metasploit, Burp Suite, Nessus            |
| DevOps & CI/CD         | Docker, GitHub Actions, Jenkins           |

---

🎓 Certifications

![CEH]
![CISSP]
![Security+]

📫 Get in Touch

💬 Email: 7ob@ro.ru

---

*"Turning challenges into solutions, one byte at a time."*

